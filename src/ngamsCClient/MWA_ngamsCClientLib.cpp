...
char* _ngamsLicense(void) {
	return const_cast<char *>("GPL");
}
char* _ngamsManPage(void) {
	return const_cast<char *>("Man page");
};
...
void ngamsInitApi(void) {
	...
	// Dave Pallot: log mutex was never initialised
...
ngamsSTAT ngamsHandleStatus(int retCode, ngamsHTTP_DATA* repDataRef, ngamsSTATUS* status) {
	...
	//printf("IN HANDLE STATUS RETCODE %d\n", retCode);
	// Dave Pallot: if its not successful then dont copy ref to the buffer
	if (repDataRef->pdata != NULL) {
		//printf("HANDLE STATUS DATA GOOD LEN: %d\n", strlen(repDataRef->pdata));
		ngamsUnpackStatus(repDataRef->pdata, status);
	}
	else {
		//printf("HANDLE STATUS DATA NULL\n");
		ngamsStat2Str((ngamsSTAT) retCode, status->message);
		if ((retCode != ngamsSRV_OK) && ((retCode != ngamsSTAT_SUCCESS)))
			strcpy(status->status, NGAMS_FAILURE);
		else
			strcpy(status->status, NGAMS_SUCCESS);

		status->errorCode = (ngamsSTAT) retCode;
	}
...
ngamsSTAT ngamsGenSendData(const char* host, const int port, const ngamsCMD cmdCode, const float timeoutSecs, const char* fileUri, const char* mimeType,
	...
	if ((strstr(fileUri, "file:") != NULL) || (strstr(fileUri, "http:") != NULL) || (strstr(fileUri, "ftp:") != NULL)) {
		...
		if ((retCode = ngamsHttpPost(host, port, ngamsUSER_AGENT, cmd, mtBuf, contDisp, fileUri, "", 0, &repDataRef, &repDataLen, &httpResp, httpHdr))!= ngamsSTAT_SUCCESS) {
			ngamsLogError("ngamsHttpPost Error: %d", retCode);
			goto errExit;
		}

			//printf("GEN SEND POST SUCCESS\n");
	}
...
ngamsSTAT ngamsArchiveFromMem(const char* host, const int port, const float timeoutSecs, const char* fileUri, const char* bufPtr, const int size,
	...
	if (*mimeType != '\0')
		strcat(tmpUrl, mtBuf);
	if ((retCode = ngamsHttpPost(host, port, ngamsUSER_AGENT, "ARCHIVE", ngamsARHIVE_REQ_MT, contDisp, "", bufPtr, size, &repDataRef, &repDataLen, &httpResp,httpHdr)) != ngamsSTAT_SUCCESS)
		goto errExit;
...
int ngamsRecvData(char** data, ngamsDATA_LEN dataLen, int* sockFd) {
	...
	ngamsDATA_LEN dataAccum = 0;


	ngamsLogDebug("Entering ngamsRecvData() ...");
	if (!(*data = (char*) malloc(dataLen + 1))) {
	...
	while (dataRem > 0) {
		...
		dataRead = read(*sockFd, (*data+dataAccum), dataRem);
		if (dataRead < 0) {
			ngamsLogError("ngamsRecvData Error: %d", dataRead);
	...
	*sockFd = 0;
	//printf("READ SUCCESS GOT %d ASKED %d\n", dataAccum, dataLen);
	ngamsLogDebug("Leaving ngamsRecvData(). Status: %d", stat);
	return stat;
	
	errExit: 
	//printf("ERROR IN READ\n");
	ngamsLogDebug("Leaving ngamsRecvData()/FAILURE. Status: %d", stat);
	return stat;
}
	...
	if (count == 100) {
		...
		for (i = 0; i < MAX_CON_THR_HANDLES; i++) {
			...
			if (((timeNow.tv_sec - thrHandleP->reservedTime.tv_sec) > THR_HANDLE_TIMEOUT) && (thrHandleP->connectFctLock || thrHandleP->connectThrLock)) {
				/* Release this entry */
				ngamsLogInfo((ngamsLOG_LEVEL) 1, ">>>>> releasing handle# %d", thrHandleP->handleNumber);
...
void* _connectThread(void* ptr) {
	...
	ngamsLogInfo((ngamsLOG_LEVEL) 1, ">>>>> par thr id: %lu, thr id: %lu: entering _connectThread()", thrInfo->parentThreadId, pthread_self());
	int n;
	for (n = 1; n <= 5; n++) {
		ngamsLogInfo((ngamsLOG_LEVEL) 1, "_connectThread(): connecting socket %d to server (try %d)", thrInfo->sockFd, n);
	...
	ngamsLogInfo((ngamsLOG_LEVEL) 1, ">>>>> par thr id: %lu, thr id: %lu: _connectThread(): connected=%d", thrInfo->parentThreadId, pthread_self(), connected);
...
int ngamsPrepSock(const char* host, const int port) {
		...
		while (1) {
			...
			if (sockFd > 0){
				...
				if ( existedSock(sockFd) ) {
						ngamsLogInfo((ngamsLOG_LEVEL) 1, "ngamsPrepSock gets the same socket(%d)", sockFd);
					continue;
				}
	...
	ngamsLogInfo((ngamsLOG_LEVEL) 1, "connecting socket(%d) to NGAS(%s:%d).", sockFd, host, port);
	if( connect(sockFd, (struct sockaddr*)&servAddr, sizeof(struct sockaddr_in)) < 0){
...
int ngamsRecvHttpHdr(int* sockFd, ngamsHTTP_HDR httpHdr, ngamsHTTP_RESP* httpResp, ngamsHTTP_DATA* repDataRef, ngamsDATA_LEN* dataLen) {
	...
	strP1 = httpHdr[0];
	strP2 = strchr(strP1, ' ');
	if ((unsigned int) (strP2 - strP1) >= sizeof(ngamsSMALL_BUF)) {
	...
	if (httpResp->status == ngamsSRV_INV_QUERY) {
		if (ngamsRecvData(&tmpData, *dataLen, sockFd) == ngamsSTAT_SUCCESS) {
			repDataRef->pdata = (char*) calloc((*dataLen + 1), sizeof(char));
...
int _ngamsHttpPost(const char* host, const int port, const char* userAgent, const char* path, const char* mimeType, const char* contentDisp,
		...
		ngamsHTTP_HDR httpHdr) {
	char header[ngamsBUFSIZE], inBuf[65536];
	...
	*repDataLen = 0;
	...
	if (*srcFilename != '\0') {
		if (stat(srcFilename, &statBuf) == -1) {
			...
			ngamsLogInfo(LEV1, "Size of file to send: %s, is: %llu bytes", srcFilename, contLen);
		}
	} else if (dataLen >= 0)
		contLen = dataLen;
		...
		if (*srcFilename != '\0') {
			...
			if ((fileFd = open(srcFilename, O_RDONLY)) == -1) {
				...
				goto errExit;
                        ...
                        ngamsLogDebug("Entering socket sending ..."); //added by Chen Wu (chen.wu@icrar.org)
			while ((bytesRead = read(fileFd, inBuf, 65536)) > 0) { //2097152
            //  while ((bytesRead = read(fileFd, inBuf, 2097152)) > 0) {
				...
				//}
                                ...
                                if (write(sockFd, inBuf, bytesRead) == -1) { //added by Chen Wu (chen.wu@icrar.org) to check socket problems
                                	ngamsLogDebug("Error writing file(%s) to socket(%d) while sending data. errno(%d):%s", srcFilename, sockFd, errno, strerror(errno));
                                	retCode = ngamsERR_WR_DATA;
                                	goto errExit;
                                }
			}
			ngamsLogDebug("Leaving socket sending ..."); //added by Chen Wu (chen.wu@icrar.org)
			// Dave Pallot: why do this?
			if (bytesRead < 0) {
				ngamsLogError("Error reading file(%s) while sending data to socket(%d). errno(%d):%s", srcFilename, sockFd, errno, strerror(errno));
				//usleep(20);
	...
	//printf("BEFORE HEADER DATA LEN %d\n", *repDataLen);
	...
	if ((retCode = ngamsRecvHttpHdr(&sockFd, httpHdr, httpResp, repDataRef, repDataLen)) != ngamsSTAT_SUCCESS) {
		ngamsLogError("ngamsRecvHttpHdr Error: %d\n", retCode);
		goto errExit;
	}

	//Dave Pallot
	//printf("AFTER HEADER DATA LEN %d\n", *repDataLen);

	/* If specified, read the data from the socket and store it into
	 * a buffer of memory allocated.
	 */
	ngamsLogDebug("Finish sending data. Try to get reply's data from server...");
	//printf("Finish sending data. Try to get reply's data from server...\n");
	//if (repDataLen) { // Dave Pallot: check its > 0
	if (*repDataLen > 0) {
		...
		//printf("SUCCESS GETTING REPLY\n");
		//printf("PAYLOAD STR LEN: %d", strlen(repDataRef->pdata));
	}
	if (sockFd > 0){
		close(sockFd);
		if ( unregisterSock(sockFd) ) ngamsLogError("fail to unregister socket(%d) in the list", sockFd); 
...
void ngamsLog_v(const char* type, const ngamsLOG_LEVEL level, const char* format, va_list vaParList) {
	...
	sprintf(logMsg, "%s [%s] %s [%lu]\n%s", isoTime, type, tmpLogMsg, (long unsigned int) pthread_self(), "\0");
...
void ngamsInitLogConds() {
	...
	if (!initialized) {
		...
		if ((charP = getenv(ngamsLOG_VERBOSE_ENV)) != NULL)
			ngamsSetVerboseLevel((ngamsLOG_LEVEL) atoi(charP));
		initialized = 1;
	}
}
...
ngamsSTAT ngamsLogFileRotate(const ngamsLOG_LEVEL tmpLogLevel, const ngamsMED_BUF systemId, ngamsMED_BUF rotatedLogFile) {
	...
	*rotatedLogFile = '\0';
	...
	if (((timeEpoch.tv_sec - secsLastRot) > (24 * 3600)) && (secsOfDay >= (unsigned int) _logRotate)) {
		...
		ngamsPrepLog(_logFile, (ngamsLOG_LEVEL) _logLevel, _logRotate, _logHistory);
...
ngamsSTAT ngamsCleanUpRotLogFiles(const ngamsLOG_LEVEL tmpLogLevel) {
	...
	while ((dirEnt = readdir(dirPtr)) != NULL) {
		if (strstr(dirEnt->d_name, ngamsLOG_ROT_PREFIX) != NULL) {
			...
			if (daysSinceLastRot >= (unsigned int) _logHistory) {
...
void ngamsSetAuthorization(const char* authUserPass) {
	_authorization = (char*) malloc(strlen(authUserPass) + 1);
...
void ngamsGenIsoTime(const int prec, ngamsMED_BUF dayTime) {
	...
	gettimeofday(&timeEpoch, NULL);
	gmtime_r((time_t*)&timeEpoch.tv_sec, &timeNow);
	asctime_r(&timeNow, ascTime);
	strcpy(tmpBuf, ascTime);
...
ngamsSTAT ngamsGetXmlAttr(const char* xmlDoc, const char* pt, const char* attr, const int maxValLen, char* value) {
	...
	if ((ptP = strstr((char*)xmlDoc, (char*)tmpPt)) == NULL)
...
ngamsSTAT ngamsLoadFile(const char* filename, char* buf, const int maxSize) {
	...
	// Dave Pallot: has to be maxSize - 1 to hold the null character!
	size = read(fd, buf, maxSize-1);
	//Dave Pallot: you have to check read!
	if (size < 0) {
		ngamsLogError("Error ocurred reading file: %s", filename);
		close(fd);
		return ngamsSTAT_FAILURE;
	}
...
ngamsSTAT ngamsSaveInFile(const char* filename, const char* buf) {
	...
	//printf("BUFF SIZE: %d STR SIZE: %d\n", sizeof(buf), strlen(buf));

	if (write(fd, buf, strlen(buf)) == -1) { //added by Chen Wu (chen.wu@icrar.org)
		ngamsLogDebug("Error writing file(%s). errno(%d):%s", filename, errno, strerror(errno));
		//retCode = ngamsERR_WR_DATA;
		//goto errExit;
		close(fd);
		return ngamsSTAT_FAILURE;
...
ngamsSTAT ngamsSplitSrvAddr(const char* srvAddr, char* host, int* port) {
	...
	if ((chrPtr = strchr((char*)srvAddr, ':')) == NULL)
		return ngamsSTAT_FAILURE;
...
ngamsSTAT ngamsSplitParVal(const char* parVal, char* par, char* val) {
	...
	if ((chrPtr = strchr((char*)parVal, '=')) == NULL)
		return ngamsSTAT_FAILURE;
...
void ngamsTrimString(char* str, const char* trimChars) {
	...
	while ((*strPtr != '\0') && (strchr(trimChars, *strPtr) != NULL))
		strPtr++;
	tmpStr = (char*) malloc(strlen(strPtr) + 1);
