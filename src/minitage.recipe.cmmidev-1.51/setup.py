...
long_description = (
    read('README.txt')
    + '\n'
    + read('src', 'minitage', 'recipe', 'cmmidev', 'tests', 'cmmidev.txt')
    + '\n'
    + read('CHANGES.txt')
    + '\n'
)
...
setup(
    ...
    package_dir = {'': 'src'},
    namespace_packages=['minitage', 'minitage.recipe', 'minitage.recipe.cmmidev'],
    ...
    entry_points = {
        'zc.buildout' : [
            'cmmi = %s:Recipe' % 'minitage.recipe.cmmidev.cmmidev',
            'default = %s:Recipe' % 'minitage.recipe.cmmidev.cmmidev',
        ]
    },
)
