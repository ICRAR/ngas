...
def ngamsGenRegPlugIn(server_object, request_object, param_dict):
    """
    ...
    (ngamsDapiStatus)
    """
    logger.info("Generic register plug-in registering file with URI: %s", request_object.getFileUri())
    disk_info = request_object.getTargDiskInfo()
    stage_file = request_object.getStagingFilename()
    # param_dict = ngamsPlugInApi.parseRegPlugInPars(server_object.getCfg(), request_object.getMimeType())

    file_name = os.path.basename(request_object.getFileUri())
    file_id = file_name.split('.')[0]
    file_id = file_id.replace(':', '/')

    mime_type_mappings = server_object.getCfg().getMimeTypeMappings()
    mime_type = ngamsLib.detMimeType(mime_type_mappings, request_object.getFileUri(), 1)
    if mime_type == ngamsCore.NGAMS_UNKNOWN_MT:
        mime_type = "multipart/related"

    file_size = ngamsPlugInApi.getFileSize(request_object.getFileUri())
    ...
    logger.info("Generic register plug-in finished processing file with URI %s: file_id=%s, file_version=%s, format=%s, file_size=%s",
